...
CREATE TABLE IF NOT EXISTS auth.embedded_pending_registrations (
    id UUID PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_embedded_pending_tenant_email
    ON auth.embedded_pending_registrations (tenant_id, LOWER(email));

CREATE INDEX IF NOT EXISTS idx_embedded_pending_expires
    ON auth.embedded_pending_registrations (expires_at);
...
ALTER TABLE auth.email_verifications
    ...
    REFERENCES auth.embedded_pending_registrations(id) ON DELETE CASCADE;

ALTER TABLE auth.email_verifications DROP CONSTRAINT IF EXISTS email_verifications_target;

ALTER TABLE auth.email_verifications ADD CONSTRAINT email_verifications_target CHECK (
...
CREATE INDEX IF NOT EXISTS idx_email_verif_pending_purpose
    ON auth.email_verifications (pending_registration_id, purpose)
