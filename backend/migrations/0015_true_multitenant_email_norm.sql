...
DROP INDEX IF EXISTS auth.uq_clients_client_id;

ALTER TABLE auth.users DROP CONSTRAINT IF EXISTS users_tenant_id_email_key;

ALTER TABLE auth.users
    ADD COLUMN IF NOT EXISTS email_norm TEXT
    GENERATED ALWAYS AS (lower(trim(both from email))) STORED;

CREATE UNIQUE INDEX IF NOT EXISTS uq_users_tenant_email_norm ON auth.users (tenant_id, email_norm);
